...
// 请求路径
const getAllUrl = `newer/showAllNewer/`,
    max = `/15`; // {currentPage}/{max}
...
let detailhtml = (
    <MyNavLink
        to="/signup/detail"
        className={({ isActive }) => "log-link" + (isActive ? " active" : "")}
    >
        详细信息
    </MyNavLink>
);
...
export default function SignUp() {
    ...
    useEffect(() => {
        ...
        switch (dataKind) {
                    ...
                    .then((res) => {
                        ...
                        res?.records.map((v, i) => {
                            ...
                            v.operation = (
                                <>
                                    <span
                                        className="operation-btn operation-btn-detail"
                                        onClick={(e) =>
                                            detailClick(e, navigate, wid)
                                        }
                                    >
                                        详情
                                    </span>
            ...
            // 搜索数据
            case 1: {
                SignupReq.blurSearch(
                    searchUrl + searchValue.trim() + "/" + curPage + max
                )
                    .then((res) => res.data)
                    .then((res) => {
                        res?.records.map((v, i) => {
        ...
        <>
            <div className="link-header">
                {pathname === "/signup" ? (
                    ...
                    <>
                        <MyNavLink to="" className="link-btn-unactive">
                            报名信息
                        </MyNavLink>
