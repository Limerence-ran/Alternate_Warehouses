...
export default function Result(props) {
    ...
    const SetModalContent = (op) => {
        ...
        switch (op) {
            ...
            case 0:
            case 4:
                setModalcontent((value) => <p>确定要删除吗？</p>);
                break;
            // 1 是打分
            case 1:
                setModalcontent((value) => conditionInput);
                break;
            // 2 是通过考试
            case 2:
                setModalcontent((value) => <p>确定要通过吗？</p>);
                break;
            // 3 评价
            case 3:
                setModalcontent((value) => {
                    ...
                    return (
                        <div className="modal-content-update">
                            <p>
                                <span className="bold-title">姓名：</span>
                                {data.newerName}
                            </p>
                            <p>
                                <span className="bold-title">轮次：</span>
                                {data.examRound}
                            </p>
                            <MyForm
                                getFormData={getFormData}
                                senddata={senddata}
                                examRound={data.examRound}
                            ></MyForm>
                        </div>
                    );
                });
            ...
            // 发送通知
            case 5:
                setModalcontent((value) => <p>确定要发送吗？</p>);
                break;
            default:
        }
    };
    ...
    const handleTableData = (data) => {
        ...
        data.map((v, i) => {
            ...
            v.operation = (
                <>
                    <span
                        className="operation-btn operation-btn-update"
                        onClick={() => updateOne(v.resultId)}
                    >
                        修改
                    </span>
                    <span
                        className="operation-btn operation-btn-delete"
                        onClick={() => deleteOne(v.resultId)}
                    >
                        删除
                    </span>
                </>
        ...
        <>
            ...
            <div className="result-ct">
                <div className="result-header">
                    ...
                    <Search
                        bordered={false}
                        placeholder="姓名/学号/学院/组别"
                        loading={loading}
                        onSearch={onSearch}
                        style={{ width: 190, height: 32 }}
                    />
                    <MySelect
                        sendvalue={(value) => sendvalue(1, value)}
                        bordered={false}
                        options={College}
                        defaultValue="全部"
                    ></MySelect>
                    <MySelect
                        sendvalue={(value) => sendvalue(2, value)}
                        bordered={false}
                        options={Groups}
                        defaultValue="全部"
                    ></MySelect>
                    <MySelect
                        sendvalue={(value) => sendvalue(3, value)}
                        bordered={false}
                        options={TestType}
                        defaultValue="笔试"
                    ></MySelect>
                    <MySelect
                        sendvalue={(value) => sendvalue(4, value)}
                        bordered={false}
                        options={Pass}
                        defaultValue="未面试"
                    ></MySelect>
                    <MySelect
                        sendvalue={(value) => sendvalue(5, value)}
                        bordered={false}
                        options={Notice}
                        defaultValue="已通知"
                    ></MySelect>
