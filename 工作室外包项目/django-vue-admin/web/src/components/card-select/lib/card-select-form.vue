  ...
  <div class="cardSelectForm">
    <div class="menu" v-for="(data, title) in tableData" :key="title">
      <div class="title" v-if="showGroupTitle">
        {{ title }}
        <span class="content"> 添加{{ title }} </span>
      </div>
      <el-row>
        <el-col
          :span="7"
          ...
          :key="index"
          class="package_features"
        >
          <div
            class="menu-item flex"
            :class="{
              active: value.indexOf(item.id) !== -1,
              disabled: item[fields.required],
            }"
...
export default {
  ...
  methods: {
    ...
    getDict() {
      ...
      if (
        that._elProps.tableConfig.data === undefined ||
        that._elProps.tableConfig.data.length === 0
      ) {
        request({
          url: url,
          method: "get",
          ...
          data = data.reduce((acc, item) => {
            ...
            const group = this.isGroup
              ? item[this.fields.group]
              : this.defaultGroupTitle;
            if (!acc[group]) {
              acc[group] = [];
            }
            acc[group].push(rest);
