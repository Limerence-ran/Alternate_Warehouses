  ...
  <div class="d2p-file-uploader" :class="{ 'is-disabled': disabled }">
    ...
    <el-dialog :visible.sync="dialogVisible" v-bind="preview" append-to-body>
      ...
      <div
        ...
        v-else-if="_elProps.listType === 'picture-card'"
      >
        <img style="max-width: 100%" :src="dialogImageUrl" alt="" />
      </div>
  ...
  computed: {
    ...
    avatarUrl() {
      if (this.fileList.length > 0) {
        const file = this.fileList[0]; //至少一个文件上传
  ...
  methods: {
    ...
    initValue(value) {
      ...
      for (const item of fileList) {
        if (item.value == null) {
          //对于每个文件对象，会对其 value 属性进行空值检查，如果为空，则将其赋值为 url 属性的值，并通过 buildUrl 方法构建文件的 URL 地址
          item.value = item.url;
        }
        //通过value构建url
    ...
    /**
     * @description fileList进行重构
     */
    resetFileList(fileList) {
      this.$set(this, "fileList", fileList);
    },
    /**
     * @description 文件上传进度处理函数
     * @param {*} event
     * @param {*} file
     * @param {*} fileList
     */
    ...
    handleUploadFileSuccess(res, file, fileList) {
      ...
      for (const item of fileList) {
        ...
        if (
          ...
          item.url.indexOf(this.baseURL) !== 0
        ) {
    ...
    /**
     * @description 触发input自定义事件并传递参数/处理单值情况
     * @param {*} res
     * @param {*} list
     */
    emit(res, list) {
    ...
    /**
     * @description 触发input自定义事件并传递参数/处理多值情况
     * @param {*} list
     */
    emitList(list) {
      if (list) {
    ...
    /**
     * @description 处理http请求函数
     * @param {*} option
     */
    httpRequest(option) {
      Promise.all([this.doUpload(option), this.computeMd5(option.file)]).then(
        (ret) => {
          // 得到组合结果，size，md5
          const result = ret[0];
          result.md5 = ret[1];
