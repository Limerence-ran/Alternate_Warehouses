<template>
  <div>
    <el-row>
      ...
      <el-col
        :span="elProps.fields[key].span"
        v-for="(_, key) in elProps.fields"
        :key="key"
      >
    ...
    >
      <el-row
        ...
        :gutter="5"
        v-for="(field, index) in currentForm.data"
        :key="index"
      >
        <el-col :span="elProps.index.span" v-if="elProps.index">
        ...
        >
          <el-form-item
            ...
            :rules="[
              {
                required: elProps.fields[key].required,
                message: '不能为空',
                trigger: 'blur',
              },
            ]"
          ...
          >
            <el-select
              v-model="field[key]"
              v-if="elProps.fields[key].type === 'select'"
              placeholder="请选择"
            >
              <el-option
            ...
            <el-input-number
              style="width: 100%"
              v-else-if="elProps.fields[key].type === 'number'"
              controls-position="right"
              v-model="field[key]"
            ></el-input-number>
            <div
              class="d2p-images"
              v-else-if="elProps.fields[key].type === 'image'"
            >
              <d2p-file-uploader
                v-model="field[key]"
                :elProps="
                  elProps.fields[key].elProps || {
            ...
            <span v-else-if="elProps.fields[key].type === 'ueditor'">
              <values-popover
                v-model="field[key]"
                :elProps="{ type: 'ueditor' }"
                @previewClick="previewClick(index, key)"
              ></values-popover>
            </span>
            <!--     多对多     -->
            <span v-else-if="elProps.fields[key].type === 'many_to_many'">
              <values-popover
                ...
                :elProps="{
                  type: elProps.fields[key].value?.type || 'manyToMany',
                  rowKey: elProps.fields[key].value?.rowKey || 'title',
                  label: elProps.value?.title || '答复选项内容',
                }"
        ...
        <el-col :span="4">
          <el-form-item>
            ...
            ></el-button>
            <el-button
              @click.prevent="bottomDomain(index)"
              :disabled="index === currentForm.data.length - 1"
              type="primary"
              circle
              icon="el-icon-bottom"
            ></el-button>
    ...
    <el-dialog
      ...
      append-to-body
      v-if="
        currentForm.data && currentForm.data[manyToManyIndex] && manyToManyKey
      "
    ...
    >
      ...
      ></foreign-key-crud-form>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="manyToManyVisible = false"
          >保存</el-button
        >
      </span>
    </el-dialog>
