  ...
  actions: {
    ...
    async openedLoad({ state, commit, dispatch }) {
      ...
      state.opened = value
        .map((opened) => {
          // 忽略控制台
          if (opened.fullPath === "/index" || opened.fullPath === "/video") {
            valid.push(1);
            return opened;
          }
    ...
    async closeLeft({ state, commit, dispatch }, { pageSelect } = {}) {
      ...
      if (currentIndex > 0) {
        ...
        for (let i = state.opened.length - 1; i >= 0; i--) {
          if (
            state.opened[i].name === "index" ||
            state.opened[i].name === "video" ||
            i >= currentIndex
          ) {
            continue;
          }
    ...
    async closeAll({ state, commit, dispatch }) {
      ...
      if (
        router.app.$route.name !== "index" ||
        router.app.$route.name !== "video"
      ) {
