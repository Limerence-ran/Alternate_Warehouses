<template>
  <d2-container>
      ...
      <div slot="widgetsList">
        ...
        <div class="widgetsListBox">
          <span v-for="item in myCompsList" :key="item.title">
            ...
            >
              <div
                class="widgetsListItem"
                :style="{ background: $util.randomBackground() }"
              >
                <span
                  style="
                    position: relative;
    ...
    <div class="widgets" ref="widgets">
      <div
        :class="['widgets-wrapper', customizing ? 'widgets-wrapper-bg' : '']"
      >
        ...
        >
          ...
          >
            <div v-if="customizing" class="customize-overlay">
              <el-button
                v-if="item.config && Object.keys(item.config).length !== 0"
                class="close"
                style="right: 60px"
                type="primary"
                plain
            ...
            <component
              :class="customizing ? 'set-component-bg' : ''"
              :is="allComps[item.element]"
              :config="item.config || {}"
              :width="item.w"
              :height="item.h"
              :pxData="pxData[item.i]"
            ></component>
          </grid-item>
        </grid-layout>
      </div>
...
export default {
  ...
  async created() {
    this.layout = await this.$store.dispatch(
      "d2admin/db/get",
      {
        dbName: "sys",
        path: "grid-layout",
  ...
  methods: {
    ...
    clickConfig(itme) {
      ...
      this.$refs.dashboardConfig.initData(
        this.allComps[itme.element],
        JSON.parse(JSON.stringify(itme))
      );
      this.minimize = false;
    },
    // 设置实际的宽度和高度
