...
util.baseURL = function () {
  ...
  if (
    window.pluginsAll &&
    window.pluginsAll.indexOf("dvadmin-tenants-web") !== -1 &&
    (!param || baseURL.startsWith("/"))
  ) {
...
util.baseFileURL = function () {
  if (
    process.env.VUE_APP_FILE_ENGINE &&
    (process.env.VUE_APP_FILE_ENGINE === "oss" ||
      process.env.VUE_APP_FILE_ENGINE === "cos")
  ) {
    return "";
  }
  return util.baseURL();
