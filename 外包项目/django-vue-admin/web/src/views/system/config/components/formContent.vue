<template>
  <div>
    <!-- 第一行标题 -->
    <el-row :gutter="10">
      <el-col :span="3">变量标题</el-col>
      <el-col :span="5" style="text-align: center">变量值</el-col>
      <el-col :span="6" style="text-align: center">变量名</el-col>
      <el-col :span="5" style="text-align: center">是否前端配置</el-col>
    ...
    >
      ...
      <el-form-item
        ...
        label-width="12.5%"
        :prop="
          ['array'].indexOf(item.form_item_type_label) > -1 ? '' : item.key
        "
        :key="index"
        :rules="item.rule || []"
      ...
      >
        ...
        <el-col :span="5">
          ...
          <el-date-picker
            v-else-if="
              ['datetime', 'date', 'time'].indexOf(item.form_item_type_label) >
              -1
            "
            v-model="form[item.key]"
            :key="index"
          ...
          <!--     图片     -->
          <div
            v-else-if="['img', 'imgs'].indexOf(item.form_item_type_label) > -1"
            :key="index"
          >
            <el-upload
              :action="uploadUrl"
          ...
          <!--     文件     -->
          <div
            v-else-if="['file'].indexOf(item.form_item_type_label) > -1"
            :key="index"
          >
            <el-upload
              :action="uploadUrl"
          ...
          <div
            v-else-if="
              ['foreignkey', 'manytomany'].indexOf(item.form_item_type_label) >
              -1
            "
            :key="index"
          >
            <table-selector
              ...
              :dict="{
                url:
                  '/api/system/system_config/get_table_data/' + item.id + '/',
          ...
          <div v-else-if="item.form_item_type_label === 'array'" :key="index">
            ...
            <div>
              <el-button size="mini" @click="onAppend('xTable_' + item.key)"
                >追加</el-button
              >
            </div>
          </div>
      ...
      <!-- 确定按钮 -->
      <el-form-item
        style="margin-left: 0; display: flex; justify-content: flex-end"
      >
  ...
  methods: {
    ...
    onSubmit() {
      ...
      that.$refs.form.validate((valid) => {
        if (valid) {
          api.saveContent(this.options.id, submitForm).then((res) => {
