...
Vue.use(d2CrudPlus, {
  ...
  getRemoteDictFunc(url, dict) {
    ...
    return request({
      ...
      if (dict.isTree) {
        let arrayTreeDept = XEUtils.toArrayTree(ret.data.data || ret.data, {
          parentKey: "parent",
          strict: false,
        });
        localStorage.setItem("dept_belong_id", JSON.stringify(arrayTreeDept));
        return arrayTreeDept;
      } else {
...
Vue.use(D2pUploader, {
  ...
  form: {
    action: util.baseURL() + "api/video_eval/create_video/",
    name: "file",
    data: {}, // 上传附加参数
    headers() {
      return {
...
Vue.prototype.commonEndColumns = function (param = {}) {
  ...
  const showData = {
    ...
    description: {
      showForm:
        (param.description && param.description.showForm) !== undefined
          ? param.description.showForm
          : true,
      showTable:
        (param.description && param.description.showTable) !== undefined
          ? param.description.showTable
          : false,
    },
    //部门
    dept_belong_id: {
      showForm:
        (param.dept_belong_id && param.dept_belong_id.showForm) !== undefined
          ? param.dept_belong_id.showForm
          : false,
      showTable:
        (param.dept_belong_id && param.dept_belong_id.showTable) !== undefined
          ? param.dept_belong_id.showTable
          : false,
      showSearch:
        (param.dept_belong_id && param.dept_belong_id.showSearch) !== undefined
          ? param.dept_belong_id.showSearch
          : false,
    },
    //修改人
    modifier_name: {
      showForm:
        (param.modifier_name && param.modifier_name.showForm) !== undefined
          ? param.modifier_name.showForm
          : false,
      showTable:
        (param.modifier_name && param.modifier_name.showTable) !== undefined
          ? param.modifier_name.showTable
          : false,
    },
    //修改时间
    update_datetime: {
      showForm:
        (param.update_datetime && param.update_datetime.showForm) !== undefined
          ? param.update_datetime.showForm
          : false,
      showTable:
        (param.update_datetime && param.update_datetime.showTable) !== undefined
          ? param.update_datetime.showTable
          : true,
    },
    //创建者
    creator_name: {
      showForm:
        (param.creator_name && param.creator_name.showForm) !== undefined
          ? param.creator_name.showForm
          : false,
      showTable:
        (param.creator_name && param.creator_name.showTable) !== undefined
          ? param.creator_name.showTable
          : false,
    },
    //创建时间
    create_datetime: {
      showForm:
        (param.create_datetime && param.create_datetime.showForm) !== undefined
          ? param.create_datetime.showForm
          : false,
      showTable:
        (param.create_datetime && param.create_datetime.showTable) !== undefined
          ? param.create_datetime.showTable
          : true,
    },
  };
  return [
    ...
    },
    {
      ...
      form: {
        ...
        helper: {
          render(h) {
            return (
              <el-alert title="默认不填则为当前创建用户的部门ID" type="info" />
            );
          },
        },
      },
