...
router.beforeEach(async (to, from, next) => {
  ...
  if (token && token !== "undefined") {
    ...
    if (
      ...
      if (
        to.path !== "/clientRenew" &&
        store.state.d2admin.user.info.tenant_id
      ) {
        // 如果租户到期，跳转到续费页面
        if (store.state.d2admin.user.info.tenant_expire) {
          ...
          store.state.d2admin.user.info.tenant_id !== 100000 &&
          !store.state.d2admin.user.info.package_manage &&
          store.state.d2admin.user.info.tenant_experience
        ) {
          next({ path: "/clientRenew" });
          // 取消当前导航
          NProgress.done();
      ...
      if (to.name) {
        if (
          to.meta.openInNewWindow &&
          ((from.query.newWindow && to.query.newWindow !== "1") ||
            from.path === "/")
        ) {
