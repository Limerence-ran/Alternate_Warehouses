...
import qs from "qs"; // qs库能将JavaScript对象转换为查询字符串

/**
 * @description 集中式处理失败的返回信息
 */
...
function createService() {
  ...
  service.interceptors.response.use(
    ...
    async (response) => {
      ...
      //对response进行初步判断：检查是否为本项目后端接口，这个状态码是和后端约定的
...
/**
 * @description 订制化formData类型请求
 * @params {Object} service axios 实例
 */
function createFormDataRequestFunction(service) {
  // 校验是否为租户模式。租户模式把域名替换成域名加端口
  return function (config) {
    const token = util.cookies.get("token");
    // 进行布尔值兼容
    ...
    return service(Object.assign(configDefault, config, { data: formData }));
  };
}
...
export const requestForFormData = createFormDataRequestFunction(service);
