...
import { softwares} from '../../../utils/data/software.js'
@Component({
  components: {
    list,
    layer
...
export default class Software extends Vue {
  ...
  mounted() {
    this.currentPage = 1;
    this.totalPage = Math.ceil(softwares.length);
  }
  ...
  changeCurrentPage(newVal) {
    ...
    for (let i = (newVal - 1) * 10; i < (newVal) * 10 && i < softwares.length; i++) {
      this.dispalyList.push(softwares[i]);
    }
