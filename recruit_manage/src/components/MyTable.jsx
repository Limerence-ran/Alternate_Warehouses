...
export default class MyTable extends Component {
  ...
  Operate(operation,dataSource,selectedRowKeys){
    ...
    switch(op){
      case 0:
        if(this.warningEmptyArr(selectedRowKeys,"还没有选择任何值噢！")) break;
      ...
      case 1:
        if(this.warningEmptyArr(selectedRowKeys,"还没有选择任何值噢！")) break;
        this.updateColumn(columns[index].dataIndex,dataSource,updatedata,selectedRowKeys);
  ...
  sendNotice(data,keys){
    ...
    TableReq.sendNotice(round.examRound,newerIdArr).then(res=>{
      ...
      if(data){
        ...
        message.error({content:"发生错误，请查看日志！",duration:1});
      }
    });
  }
    ...
    componentDidUpdate(prevProps, prevState){
      ...
      if(operation){
        if(operation.op !== prevState.operation.op && operation.op !== -1){
